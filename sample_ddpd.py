...
from train_ddpd import DDPDConfig, DDPDPlanner, DDPDDenoiser
import click
import os
...
@click.command()
@click.option('--checkpoint', required=True, help='Path to the trained model checkpoint')
@click.option('--decoder-path', default='/home/ubuntu/simo/nano-diffusion-speedrun/tokenize_dataset/pretrained_ckpts/Cosmos-Tokenizer-DI8x8/decoder.jit', help='Path to the JIT decoder model')
...
@click.option('--output-dir', default='samples', help='Directory to save generated samples')
@click.option('--class-label', default="0,1,2,3", type=str, help='Specific class label to generate (optional)')
...
def sample(
    ...
    if class_label is not None:
        ...
        class_labels = torch.tensor([int(label) for label in class_label.split(',')], device=device)
        num_samples = class_labels.shape[0]
    else:
        class_labels = torch.randint(0, planner.config.num_classes, (num_samples,), device=device)
